...
path_experiment = './log/experiments/' + name_experiment + '/' + dataset + '_cross_DRIVE/'
...
# 跨数据集测试：固定加载 DRIVE 的 checkpoint_epoch_011.pth（Epoch 10，充分训练且 DRIVE 测试指标中期最高）
weight_files = ['./log/experiments/DRIVE/checkpoint_epoch_011.pth']
print("loaded:" + weight_files[0])
if mode == 'cpu':
    checkpoint = torch.load(weight_files[0],
    ...
    checkpoint = torch.load(weight_files[0], map_location=('cuda:' + str(gpuid)))
    state_dict = {k: v for k, v in checkpoint['state_dict'].items()
                  if not k.endswith('.total_ops') and not k.endswith('.total_params')}
    model.load_state_dict(state_dict, strict=False)
    model.cuda()
