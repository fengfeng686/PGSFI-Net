...
class PGSFINetFrequencyOnly(nn.Module):
    """消融：完整版，但 MBSFF 只保留 Haar 频率分支（去掉空间分支和 cross-gating）"""

    def __init__(self, n_channels, n_classes, downsize_nb_filters_factor=4, dropout_rate=0.1):
        super(PGSFINetFrequencyOnly, self).__init__()
        ...
        self.spatial_freq_block = MultiBranchFrequencyOnly(
            in_dim=512 // f,
            num_blocks=1
        )
