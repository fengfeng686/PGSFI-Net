...
class PGSFINetSpatialOnly(nn.Module):
    """消融：完整版，但 MBSFF 只保留空间分支（去掉频率分支和 cross-gating）"""

    def __init__(self, n_channels, n_classes, downsize_nb_filters_factor=4, dropout_rate=0.1):
        super(PGSFINetSpatialOnly, self).__init__()
        ...
        self.spatial_freq_block = MultiBranchSpatialOnly(
            in_dim=512 // f,
            num_blocks=1
        )
