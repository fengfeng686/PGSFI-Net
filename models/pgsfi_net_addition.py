...
class PGSFINetAddition(nn.Module):
    """消融：完整版，但 MBSFF 用 Addition（F_spatial + F_freq）替换 cross-gating"""

    def __init__(self, n_channels, n_classes, downsize_nb_filters_factor=4, dropout_rate=0.1):
        super(PGSFINetAddition, self).__init__()
        ...
        self.spatial_freq_block = MultiBranchAdditionFusion(
            in_dim=512 // f,
            num_blocks=1
        )
