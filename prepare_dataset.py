...
config_path = os.path.join(script_dir, 'configuration.txt')
config.read(config_path)
dataset = config.get('data attributes', 'dataset')
dataset_dict = ['STARE', 'CHASE', 'CHASEDB1']
...
# 强制指定你的真实路径
dataset_path = "/root/autodl-tmp/PGSFI-Net/data/" + dataset + "_hdf5/"
# 打印路径，让你看到存在哪！
print("文件将保存到：", dataset_path)

# train
dataset_base = os.path.dirname(dataset_path.rstrip('/'))
label_dir = "1st_manual" if dataset == "DRIVE" else "labels"
original_imgs_train = dataset_base + "/" + dataset + "/training/images/"
groundTruth_imgs_train = dataset_base + "/" + dataset + "/training/" + label_dir + "/"
borderMasks_imgs_train = dataset_base + "/" + dataset + "/training/mask/"
# test
original_imgs_test = dataset_base + "/" + dataset + "/test/images/"
groundTruth_imgs_test = dataset_base + "/" + dataset + "/test/" + label_dir + "/"
borderMasks_imgs_test = dataset_base + "/" + dataset + "/test/mask/"
...
def get_datasets(imgs_dir, groundTruth_dir, borderMasks_dir, train_test="null"):
    ...
    files = sorted(files)  # 确保文件按字母顺序排序
    assert len(files) > 0
    ...
    for i in range(len(files)):
        ...
        if dataset == "STARE":
            groundTruth_name = files[i][:-4] + ".ah.ppm"
        if dataset == "DRIVE":
        ...
        if dataset not in dataset_dict:
            ...
            # get fov mask for STARE CHASE CHASEDB1
            threshold = 0.01
            if dataset == "STARE":
                threshold = 0.19
            ...
            # save the fov mask
            if not os.path.exists(borderMasks_dir):
                os.makedirs(borderMasks_dir)
            Image.fromarray(fov_mask * 255).convert("RGB").save(
                borderMasks_dir + files[i][:-4] + '_fov_mask.png', "png")
